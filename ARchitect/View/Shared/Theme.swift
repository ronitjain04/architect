...
/// A story bubble: gold gradient ring around an avatar with a caption below.
        ...
        VStack(spacing: 5) {
            ZStack {
                Circle()
                    .strokeBorder(
                        LinearGradient(
                            colors: [.appAccent, Color(hex: "#E0A85A"), .appAccent],
                            startPoint: .topLeading, endPoint: .bottomTrailing
                        ),
                        lineWidth: 2.5
                    )
                Avatar(systemImage: systemImage, monogram: monogram, size: size - 8)
                    .overlay(
